...
import logoBlack from '/src/images/logo/logo-black.png';
import logoWhite from '/src/images/logo/logo-white.png';
import logoMarkBlack from '/src/images/logo/logo-mark-black.png';
import logoMarkWhite from '/src/images/logo/logo-mark-white.png';
...
export function Logomark({ invert = false, filled = false, ...props }) {
  const id = useId()
  const logoSrc = invert ? logoMarkWhite : logoMarkBlack;
...
export function Logo({
  ...
  const logoSrc = invert ? logoWhite : logoBlack;

  return (
    <div
      className={clsx('flex items-center', fillOnHover && 'group/logo', className)}
      {...props}
    >
      {/* <Logomark
        preserveAspectRatio="xMinYMid meet"
      ...
      <Image 
        src={logoSrc} 
        alt="Logo" 
        className={clsx('ml-2', invert ? 'fill-white' : 'fill-neutral-950')}
      />
    </div>
  )
}